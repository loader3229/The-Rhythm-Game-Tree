    ...
    update(diff) {
     player.r.rot=layers.r.rotCal()
		 ...
		 if(hasUpgrade('j',31)&&layers.r.buyables[13].canAfford()&&layers.r.buyables[13].unlocked()) layers.r.buyables[13].buy();
			...
			if(player.r.points.lt(n(tmp.r.resetGain).mul(tmp.r.passiveGeneration).mul(player._devSpeed).mul(clickableEffect('r',161)))&&gcs('r',161)==1) player.r.points=player.r.points.add(n(tmp.r.resetGain).mul(tmp.r.passiveGeneration).mul(diff).mul(clickableEffect('r',161)).div(10))
			}
	},
	passiveGeneration()
  {mult = n(0)
 ...
 if(gcs('r',93)==1) chal=chal.div(1.8)
   ...
   tabFormat: {
    ...
    ["display-text", function() {if(gcs('r',161)==1){
     if(player.r.points.lt(n(tmp.r.resetGain).mul(tmp.r.passiveGeneration).mul(player._devSpeed).mul(clickableEffect('r',161)))) return '算上升级效果，你正在获得 ' + format(n(tmp.r.resetGain).mul(tmp.r.passiveGeneration).mul(player._devSpeed).mul(clickableEffect('r',161)).div(10))+' 旋律每秒<br>你需要'+format(player.r.points.div(clickableEffect('r',161)))+'旋律每秒以获得升级的效果'
     else return '算上升级效果，你正在获得 ' + format(n(tmp.r.resetGain).mul(tmp.r.passiveGeneration).mul(player._devSpeed))+' 旋律每秒<br>你需要'+format(player.r.points.div(clickableEffect('r',161)))+'旋律每秒以获得升级的效果'
    }},
     {"color": "#ffffff", "font-size": "14px", "font-family": "Comic Sans MS"}],"blank",
  ...
  softcapPower:new Decimal(1),
    ...
    47:{ title: "层级助推 V",
      ...
      11: {
        ...
        completionLimit(){
          return new Decimal(5).add(hasUpgrade('lo',101)?15:0).add(hasUpgrade('lo',103)?20:0).add(hasUpgrade('lo',112)?260:0)},
...
         function() {if(hasUpgrade('j',15)) return '5.曲包需求^' + format(tmp.j.pdqja5)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
         ["display-text",
         function() {if(hasUpgrade('j',170)) return '6.谱面需求^' + format(tmp.j.pdqja6)},
     {"color": "#ffffff", "font-size": "20px", "font-family": "Comic Sans MS"}],
     "upgrades",
     ],
...
doReset(resettingLayer) {
   ...
   upgrades: {
    ...
    27:{ 
    fullDisplay() {return "“QqQe308”<br>解锁曲包专精，自动最大购买Rotaeno曲包，升级“空游呀”不再有下限e3800000<br>需求：通过414ms判定区间挑战"},
    unlocked() {return hasUpgrade('j',26) && player.lo.evolution.gte(2)},
    tooltip:"曲包专精代替了原计划的Milthm曲包",
    canAfford() {return player.j.pdqja.lte(414)},
  },
    28:{ 
    fullDisplay() {return "“qq1010903229”<br>本层级名称为loader3229的升级对S-Perfect判定也生效，解锁新的loader3229升级<br>需求：通过410ms判定区间挑战"},
    unlocked() {return player.lo.evolution.gte(2)},
    canAfford() {return player.j.pdqja.lte(410)},
  },
    31:{ 
